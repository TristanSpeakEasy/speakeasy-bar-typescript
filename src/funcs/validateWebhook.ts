...
export async function validateWebhook(_client: SpeakeasyBarCore, {
  request: rawRequest,
}: {
  request: {
    body: BodyInit;
...
}): Promise<Result<webhooks.StockUpdateRequest, SDKValidationError>> {
  const request = normalizeRequest(rawRequest);
  const knownSchemas = [
    webhooks.stockUpdateRequestFromJSON,
    webhooks.stockUpdateRequestFromJSON,
  ];
