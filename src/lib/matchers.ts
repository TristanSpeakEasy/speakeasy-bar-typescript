...
import { SpeakeasyBarDefaultError } from "../models/errors/speakeasybardefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | SpeakeasyBarDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | SpeakeasyBarDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new SpeakeasyBarDefaultError(
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new SpeakeasyBarDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
